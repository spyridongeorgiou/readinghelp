...
if __name__ == "__main__":
    ...
    pg.event.set_grab(True) # limit input to only the pygame window, to avoid accidentally interacting with programs outside the pygame window
    calibri = pg.font.SysFont("Calibri",24) # Fonttype and Size
    pg.key.set_repeat(125) # control how held keys are repeated upon being held down
...
#       display color + current color value number, keys used to increase/decrease line thickness
        vtInfo = ("Lesehilfe - Spyridon Georgiou")
        vtR = ("'1' - Rot: " + str(R)) 
        vtG = ("'2' - Grün: " + str(G))  
        vtB = ("'3' - Blau: " + str(B))
...
#       create image object which can be drawn on screen by using blit. (text,antialiasing,color(tuple),background)
        textInfo = calibri.render(vtInfo,1, (color[0],color[1],color[2]), white)
...
#       draw the previously created image object on screen. (image,(X,Y))
        screen.blit(textInfo, (0,xyvtInfo[1]*6))
